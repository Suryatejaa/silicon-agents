...
async def verify(request: VerifyRequest) -> StreamingResponse:
    ...
    logger.info(
        "Agent01 verify request accepted project=%s mode=%s artifact=%s chars=%s",
        request.project_id,
        request.mode,
    ...
    async def generator():
        ...
        status = "completed"
        logger.info("Agent01 stream started run=%s project=%s", run_id, request.project_id)
        ...
        try:
            ...
            record = RunHistoryRecord(
                ...
                raw_artifact=request.report_text,
...
def _retrieval_query(request: VerifyRequest) -> str:
    return "\n".join(
        part
        for part in [
            request.design_name,
            ...
            request.report_text[:2500],
        ]
        if part
    )


def _request_with_retrieved_context(request: VerifyRequest, documents: list) -> VerifyRequest:
    if not documents:
        return request
    retrieved_context = "\n\n".join(
    ...
    label = request.reference_data_label or "retrieved run history"
    return request.model_copy(
        update={
            "reference_data": f"{existing}\n\nRetrieved prior run evidence:\n{retrieved_context}".strip(),
